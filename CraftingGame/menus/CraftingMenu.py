...
class CraftingMenu(Menu):
    def __init__(self, x, y, width, height, edge_spacing, menu_font_size, title_spacing, title, active, inventory, item_manager, message_menu, button_manager):
    ...
    def draw(self, surface):
        ...
        if self.active:
            ...
            if self.inventory.selected_slot is not None and self.inventory.selected_slot.get_item_in_slot():
                ...
                text.draw_text(surface, self.inventory.selected_slot.get_item_in_slot().name, int(self.menu_font_size*1.5), color.white, self.info_left_x, self.info_left_upper_y)
                text.draw_text(surface, f"Amount: {self.inventory.selected_slot.amount}", self.menu_font_size, color.white, self.info_middle_x, self.info_left_lower_y + 20 + self.edge_spacing)
                text.draw_text(surface, stackable_str, self.menu_font_size, color.white, self.info_middle_x, self.info_left_lower_y + 40 + self.edge_spacing)
