...
class CraftingMenu(Menu):
    def __init__(self, x, y, width, height, edge_spacing, menu_font_size, title_spacing, title, active, inventory, item_manager):
    ...
    def draw(self, surface):
        ...
        if self.inventory.selected_slot is not None and self.inventory.selected_slot.get_item_in_slot():
            ...
            text.draw_text(surface, self.inventory.selected_slot.get_item_in_slot().name, int(self.menu_font_size*1.5), color.white, self.info_x + self.edge_spacing, self.info_y + self.edge_spacing)
            text.draw_text(surface, f"Amount: {self.inventory.selected_slot.amount}", self.menu_font_size, color.white, self.info_x + self.edge_spacing, self.info_y + self.edge_spacing + 20)
            text.draw_text(surface, stackable_str, self.menu_font_size, color.white, self.info_x + self.edge_spacing, self.info_y + self.edge_spacing + 40)
            surface.blit(image_item, (self.info_x + self.edge_spacing + 100, self.info_y + self.info_height//2 - image_item.get_height()//2))
            text.draw_multiline_text(surface, self.inventory.selected_slot.get_item_in_slot().description, self.menu_font_size,
                                     color.white,
                                     self.info_x + self.edge_spacing*2 + 100 + image_item.get_width(), self.info_y + self.edge_spacing, 200)
